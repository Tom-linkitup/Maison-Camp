...
//			try {
				...
				String itemName = req.getParameter("itemName");
				String itemNameReg = "^[(\u4e00-\u9fa5)(a-zA-Z0-9_)]{1,10}$";
				if (itemName == null || itemName.trim().length() == 0) {
					errorMsgs.add("商品名稱: 請勿空白");
				} else if (!itemName.trim().matches(itemNameReg)) { // 以下練習正則(規)表示式(regular-expression)
					errorMsgs.add("商品名稱: 只能是中、英文字母、數字和_ , 且長度必需在1到10之間");
				}
...
//				String url = "/back-end/item/listAllItem2.jsp";
		...
		if ("delete".equals(action)) { // 來自listAllEmp.jsp
			...
			try {
				...
				RequestDispatcher failureView = req
						.getRequestDispatcher("/back-end/item/ItemInfo.jsp");
				failureView.forward(req, res);
			}
		}
	}
